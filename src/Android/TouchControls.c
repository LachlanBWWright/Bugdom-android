...
// Virtual joystick + action buttons for Bugdom.
...
static float gJoyTouchX      = 0;     // touch position in pixels
...
static void UpdateButtonPositions(void)
{
    ...
    // Up = Jump, Right = Attack, Down = Kick, Left = Pickup
                              ...
                              float r, float g, float b, float a)
{
    ...
    float verts[2 + 2 * 64];    // center + segs+1 points (fan)
...
void TouchControls_Draw(void)
{
    ...
    if (gJoyActive)
    {
        float tx = gJoyTouchX;
        float ty = gJoyTouchY;
        // Clamp thumb inside joystick circle
    ...
    // Draw action buttons
    static const char *kBtnLabels[] = { "^", ">", "v", "<", "||" };
    (void)kBtnLabels;
    for (int i = 0; i < kTouchBtn_COUNT; i++)
    {
        float r = BtnRadius(i);
